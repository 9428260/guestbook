...
function setEventListener() {
    ...
    $('#role_grid').on('pageSet', function(e) {
    ...
    $('#role_grid').on('perPageChange', function(e) {
        let evObj = AlopexGrid.parseEvent(e);
        g_page = 1;
    ...
    $('#role_grid').on('dblclick', '.bodycell', function(e) {
        let dataObj = AlopexGrid.parseEvent(e).data;
        let rowData = $("#role_grid").alopexGrid( "dataGetByIndex" , {data: dataObj._index.data});

        if(rowData && rowData['id']) {
            let params = {
                'role_id'    : rowData['id'],
                'sc_role_id' : $("#role_id").val(),
                'sc_role_nm' : $("#role_nm").val(),
                'sc_page'    : g_page,
                'sc_per_page': g_per_page,
            };

            opme_postWithParam('/role/dtl', params);
        }
    });

    // 삭제
    $("#btn_del_role").on("click", function(e) {
        let data = $('#role_grid').alopexGrid('dataGet' , { _state : { selected : true } } );// 선택된 데이터
        if (data == '') return;

        if ( confirm("선택된 Role을 삭제 하시겠습니까?") ) {
            delRole(AlopexGrid.trimData(data));
        }
    });
};
...
function initGrid() {
    ...
    if (typeof request_params !== 'undefined' && request_params != null) {
        $('#role_id').val(request_params['sc_role_id']);
        $('#role_nm').val(request_params['sc_role_nm']);
...
function getGridData() {
    ...
    return $.ajax({
        ...
        success    : function(result){
            ...
            $('#role_grid').alopexGrid('dataSet', result['roleList'], serverPageInfo);
        }
    });
};
